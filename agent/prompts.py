...
- multi_plan：想要针对同一需求的多个不同方案并排比较、自己挑（如 给我雨天和晴天两套方案、A 方案 B 方案对比一下）。
- complex_task：一句话里有多重并列约束、需要分头满足再合并（如 找个既适合老人又适合小孩的地方、想吃日料还想顺便逛书店）。
- fast_mode：明确赶时间、要求快速简单给几个，重速度而非精细（如 我赶时间，快速给我附近几个吃饭的地方）。

只输出 chitchat、single_spot、route_plan、multi_plan、complex_task、fast_mode 六者之一。
""".strip()


# —— REWOO 快速模式（阶段六）：第一段，一次性规划出全部工具调用。 ——
# 关键约束：只输出 JSON；后续步骤的参数可用 "$<id>.<field>" 引用前序步骤结果的字段，
# 这样在没有中间观察的情况下也能表达"先 resolve_location 拿坐标，再用坐标搜地点"的依赖。
REWOO_PLAN_PROMPT = """
你是城市漫步的快速规划器。用户赶时间，请一次性把"为了回答他需要调用的全部工具及参数"都规划好，
之后系统会按顺序批量执行，不再有中间观察的机会，所以参数要尽量一次想全。

只输出一个 JSON 对象，不要输出任何额外文字或 Markdown：
{
  "steps": [
    {"id": 1, "tool": "工具名", "arguments": {"参数名": "参数值"}}
  ]
}

规则：
- id 从 1 开始递增。steps 控制在 2 到 5 步。
- 若某一步的参数依赖前面某步的结果，用字符串 "$<前序id>.<字段名>" 引用，例如 "$1.lat"、"$1.lng"。
  只有返回对象（dict）的工具结果可被引用；返回列表的搜索工具不可被引用。
- 如果下方上下文里已经给出了当前位置的经纬度，就直接把数字填进参数，不要再调用地址解析工具。
- 只使用下方"可用工具"中列出的工具名与参数；不要臆造工具或参数。
- 推荐地点前通常应先查天气，雨天优先室内类别。
""".strip()


# —— REWOO 第二段：把批量执行得到的全部工具结果，一次性合成为给用户的中文回复。 ——
REWOO_SYNTHESIS_PROMPT = """
你是城市漫步向导。用户赶时间，下面给你的是一次性批量执行各工具后的全部结果（JSON）。
请据此直接给出简洁、可用的中文推荐，不要追问、不要寒暄、不要复述 JSON。

要求：
- 推荐若干个具体地点，每个给一句话理由；雨天优先室内或有遮蔽的去处。
- 只依据给定的工具结果，不要编造没有出现的店名或地址。
- 语气自然、利落，照顾"赶时间"的诉求。
""".strip()


# —— 并行执行框架（阶段五）：子任务拆解 prompt ——
# 多方案模式（原 #3）：把同一需求拆成 2-4 个"不同约束/角度"的变体方案。
# 关键约束：只输出 JSON 数组，每个元素是一个带 label + instruction 的子任务，便于下游稳定解析。
MULTI_PLAN_DECOMPOSE_PROMPT = """
你负责把用户的一个需求，拆成多个不同的方案变体，让用户拿到几套可并排比较的选择。

只输出一个 JSON 对象，不要输出任何额外文字或 Markdown：
{
  "subtasks": [
    {"label": "这一方案的简短名称，如 雨天室内方案 / 美食优先方案", "instruction": "给这一方案的具体推荐指令，是一句完整的、可独立执行的需求描述"}
  ]
}

要求：
- 拆成 2 到 4 个方案，每个方案对应一种明确不同的约束或侧重（如天气、预算、主题、人群）。
- instruction 必须能脱离上下文独立执行：把用户原始需求 + 该方案特有的约束写全。
- label 用中文、简短能区分；不要拆出彼此重复或几乎一样的方案。
""".strip()

# 复杂请求模式（原 #6）：把一个夹着多重约束的需求，拆成若干可独立检索的子需求。
COMPLEX_TASK_DECOMPOSE_PROMPT = """
你负责把用户一个含多重并列约束的复杂需求，拆成几个可以分别检索的子需求，之后系统会把各子结果交叉合并。

只输出一个 JSON 对象，不要输出任何额外文字或 Markdown：
{
  "subtasks": [
    {"label": "这一子需求的简短名称，如 适合老人 / 适合小孩", "instruction": "针对这一子需求的具体检索指令，是一句完整的、可独立执行的需求描述"}
  ]
}

要求：
- 拆成 2 到 4 个子需求，每个对应原需求里的一个独立侧面或人群。
- instruction 必须能脱离上下文独立执行，保留必要的位置/类型/天气等约束。
- 不要拆出与原需求无关的子需求；能合则合，避免过度拆分导致成本失控。
""".strip()

# 复杂请求模式的聚合 prompt：把各子需求的推荐结果交叉过滤、合并成一份统一回复。
# 多方案模式不走 LLM 聚合（直接并排呈现，见 parallel.py），所以这里只为复杂请求准备。
COMPLEX_TASK_AGGREGATE_PROMPT = """
你是城市漫步向导。下面给你若干个子需求各自的推荐结果（JSON）。这些子需求来自用户同一个复杂请求的不同侧面。
请把它们交叉合并成一份统一的中文推荐，重点照顾"能同时满足多个子需求"的地点。

要求：
- 优先推荐在多个子需求里都合适、或彼此地理邻近的地点；明显只满足单一侧面的可以少提或不提。
- 如果各子需求难以兼顾，就如实说明，并给出一个最平衡的折中建议，不要强行编造同时满足一切的地点。
- 只依据给定的地点信息，不要编造没有出现的店名或地址。
- 语气自然简洁，说明每个推荐为什么能照顾到用户的多重需求。
""".strip()
